...
func TestElasticsearchToolEndpoints(t *testing.T) {
	...
	insertStmt := fmt.Sprintf("FROM %s | WHERE name == ? AND embedding IS NOT NULL | LIMIT 0", index)
...
func getElasticsearchWants() ElasticsearchWants {
	...
	nullWant := `{"error":{"root_cause":[{"type":"verification_exception","reason":"Found 1 problem\nline 1:25: first argument of [name == ?] is [text] so second argument must also be [text] but was [null]"}],"type":"verification_exception","reason":"Found 1 problem\nline 1:25: first argument of [name == ?] is [text] so second argument must also be [text] but was [null]"},"status":400}`
